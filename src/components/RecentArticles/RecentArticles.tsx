      ...
      <div className="w-[90%] flex flex-wrap justify-between gap-5">
        ...
        {recentBlogs.length > 0 && recentBlogs.map((blog) => (
          <div className="lg:w-[48%] w-full flex flex-col">
            ...
            {/* blog  category */}
            <div className={`absolute top-4 left-4 bg-${blog.category.toLowerCase()} border border-r-3 border-b-3 px-2 rounded-xl w-fit flex items-center gap-1`}>
            ...
            <div className="absolute bottom-0 left-0 w-full flex items-center justify-center">
              <div className="w-[95%] left-4 bg-white border border-r-5 border-b-5 px-2   flex items-center gap-1 my-3 p-2">
                <Link to={`/blog/${blog.id}`} className="text-4xl font-bold hover:underline hover:text-red-500 transition-all duration-700 ease-in-out cursor-pointer">
                  {blog.title}
                </Link>
              </div>
            </div>
          ...
          <div className="w-full mb-5 text-xl">
            ...
            <p>
              {blog.content.replace(/[^\w\s]/g, " ").slice(0, 100)}...
            </p>
          </div>
        </div>
        ))}
